 ...
 * fitnesses evaluated.
 */
public class RNAProblem extends Problem implements SimpleProblemForm, GroupedProblemForm
    {
    ...
    String target_phenotype;

    public void setup(final EvolutionState state, final Parameter base)
        {
            target_phenotype = state.parameters.getString(base.push(P_TARGETPHENOTYPE), null);
            if (target_phenotype == null)
                state.output.fatal(String.format("%s: no value given for parameter '%s', but we need a target phenotype.", this.getClass().getSimpleName(), base.push(P_TARGETPHENOTYPE)));
	...
	public void evaluate(EvolutionState state, Individual[] individuals, boolean[] updateFitness, boolean countVictoriesOnly,
        ...
        try
            ...
            for (int i = 0; i < individuals.length; i++)
                ...
                if(fitnesses.get(i)==0){
                    isIdeal=true;
                }
                ((SimpleFitness)ind.fitness).setFitness(state, simple_fitness, isIdeal);
                ind.evaluated = true;
                }
            }
    ...
    private String runCommand(final Individual[] individuals) throws IOException, InterruptedException
        {
            //Test write to file
            File file = new File(RNAFILE);
            FileWriter fw = new FileWriter(file);
            final Writer fileInput = new BufferedWriter(fw);
            writeIndividuals(individuals, fileInput);
            fileInput.close();
...
/*        final StringBuilder sb = new StringBuilder();
    ...
    public static void writeIndividuals(final Individual[] individuals, final Writer outWriter) throws IOException
        ...
        for (final Individual ind : individuals)
            {
            final char[] genome = ((CharVectorIndividual) ind).genome;
            assert(genome.length > 0);
            outWriter.write(String.valueOf(genome[0]));
            for (int i = 1; i < genome.length; i++)
                outWriter.write(String.format("%c", genome[i]));
                //outWriter.write(String.format("%s%c", DELIMITER, genome[i]));
